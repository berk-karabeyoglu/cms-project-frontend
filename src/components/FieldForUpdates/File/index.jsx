...
const FileUpdateField = ({ fieldObj }) => {
    ...
    useEffect(() => {
        ...
        if (fieldObj !== undefined) {
            console.log('BAK INCOMING MİME TYPES BUNLAR:', fieldObj.mimeTypes);
            var mimeTypes = '';
            var dottedArr = [];
            fieldObj.mimeTypes.map(mime => {
                dottedArr.push('.' + mime);
            });
            mimeTypes = dottedArr.join(',');
                ...
                {props => (
                    <Form>
                        ...
                        <Flex wrap={'wrap'} justifyContent={'space-evenly'}>
                            {/* Name Input */}
                            <Field name="name" validate={fileFieldValidations.validateFieldName}>
                                {({ field, form }) => (
                                    <FormControl
                                        w={'40%'}
                                        minW={'200px'}
                            ...
                            <Field name="columName">
                                {({ field, form }) => (
                                    ...
                                    >
                                        ...
                                        <Text fontSize="lg" {...field} disabled size="md" id="column_name" type="text">
                                            {defaultColumnName}
                                        </Text>
                            ...
                            <Field name="mime_types">
                                {({ field, form }) => (
                                    <FormControl
                                        w={'30%'}
                                        minW={'200px'}
                                        isInvalid={form.errors.mime_types && form.touched.mime_types}
                                        mb={5}
                                    >
                                        <FormLabel htmlFor="mime_types">Allowed Mimes</FormLabel>
                                        <Input {...field} size="sm" id="mime_types" type="text" />

                                        <FormErrorMessage>{form.errors.mime_types}</FormErrorMessage>
                                    </FormControl>
                                )}
                            </Field>
                            ...
                            <Field name="fileAmount">
                                {({ field, form }) => (
                                    <FormControl
                                        w={'30%'}
                                        minW={'200px'}
                                        isInvalid={form.errors.fileAmount && form.touched.fileAmount}
                                        mb={5}
                                    >
                                        ...
                                        <FormErrorMessage>{form.errors.fileAmount}</FormErrorMessage>
                                    </FormControl>
                                )}
                            </Field>
